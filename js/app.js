...
const contract = new web3.eth.Contract(abi, '0x343a77d4cC62fbBC0ed45BE4898aF47fB5FC4acF');
				...
				.then(function(result) {
					if (!result) {
						...
						contract.methods.isSlotBookedBySender(i, j)
							...
							.then(function(result) {
								if (result) {
									$slot.removeClass('list-group-item-danger').addClass('list-group-item-warning');
								}
								else {
									$slot.removeClass('list-group-item-warning').addClass('list-group-item-danger');
								}
							});
					}
				});
